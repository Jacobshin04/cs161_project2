...
func (userdata *User) LoadFile(filename string) (content []byte, err error) {
	...
	// Step 5: Use FileKey to decrypt the actual file metadata
...
func (userdata *User) CreateInvitation(filename, recipient string) (invitationPtr uuid.UUID, err error) {
	...
	// Step 3: Retrieve your encrypted fileKey
	keyUUIDBytes := userlib.Hash([]byte("filekey_" + filename + hex.EncodeToString(userdata.SK)))[:16]
	keyUUID, _ := uuid.FromBytes(keyUUIDBytes)
	encFileKeyBytes, ok := userlib.DatastoreGet(keyUUID)
	if !ok {
	...
	// Step 4: Decrypt FileObject with fileKey
	var encFileObjBlob EncryptedBlob
	...
	// Step 4: Create invitation struct
	invite := Invitation{
		FileUUID: fileUUID,
		FileKey:  fileObj.Key,
...
func (userdata *User) AcceptInvitation(senderUsername string, invitationPtr uuid.UUID, filename string) error {
	...
	// Step 4: Load or initialize recipient’s file map
	mapUUIDBytes := userlib.Hash([]byte("filemap" + hex.EncodeToString(userdata.SK)))[:16]
	mapUUID, err := uuid.FromBytes(mapUUIDBytes)
	if err != nil {
		return err
	...
	// Step 8: Store recipient’s encrypted copy of fileKey
	keyUUIDBytes := userlib.Hash([]byte("filekey_" + filename + hex.EncodeToString(userdata.SK)))[:16]
	keyUUID, _ := uuid.FromBytes(keyUUIDBytes)
...
func (userdata *User) RevokeAccess(filename string, targetUsername string) error {
	...
	// Step 2: Decrypt the file object
	...
	// Step 3: Generate new key for file (rotating key)
	...
	// Step 4: Re-encrypt file object with new key
	fileObj.Key = newKey
	...
	// Step 5: Update owner’s key reference
	...
	// Step 6: Remove target user’s access
	// simplest: remove their key reference so decryption fails
